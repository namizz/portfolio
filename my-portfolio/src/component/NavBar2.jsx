...
export const NavItems = ({ items, className, onItemClick }) => {
  ...
  const scrollToSection = (id) => {
    ...
    if (element) {
      // Use window.scroll() instead of scrollIntoView
      window.scrollTo({
        top: element.offsetTop - 60, // Adjust for fixed navbar height (60px)
  ...
  const handleClick = (e, link) => {
    if (link.startsWith("#")) {
      ...
      const id = link.substring(1); // Remove '#'
      scrollToSection(id);
    }
...
export const NavbarLogo = () => {
  return (
    ...
    >
      ...
      <span className="text-md font-bold text-black ">Mr. Shiferaw</span>
    </a>
  );
};
...
export const NavbarButton = ({
  ...
  variant = "primary",
  className,
  download = false, // <-- added download prop
  ...props
}) => {
  const baseStyles =
  ...
  return (
    <div className="my-auto" onClick={handleClick}>
      <a
        href={href || "#"}
        download={download} // <-- support download
